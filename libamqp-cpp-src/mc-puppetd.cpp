    ...
    virtual void
      run ()
      {
        ...
        try
        {
          ...
          if (useTopic)
          {
            destination = session->createTopic ("mcollective.puppetd.command");
          }
          else
          {
...
*/
              ...
              bodydoc[":action"] >> action;
              std::cout << action;


              // Construct YAML body
              YAML::Emitter reply_message_body_yaml;
              reply_message_body_yaml << YAML::BeginMap;
              reply_message_body_yaml << YAML::Key << ":output";
              reply_message_body_yaml << YAML::Value << "lock removed";
              reply_message_body_yaml << YAML::EndMap;
              ...
              reply_message_yaml << YAML::Value << "puppetd";
              reply_message_yaml << YAML::Key << ":msgtarget";
              reply_message_yaml << YAML::Value << "/topic/mcollective.puppetd.reply";
              ...
              try {
                ...
                session = connection->createSession( Session::AUTO_ACKNOWLEDGE );
                // Create the destination (Topic or Queue)
                destination = session->createTopic( "mcollective.puppetd.reply" );
                ...
                reply->writeString(reply_message);
                producer->send( reply );
                printf("reply send \n");
